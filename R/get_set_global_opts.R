...
#'
#' Returns path to current style sheet which is set in global options. To change this path see \link[xltabr]{set_style_path}.
...
#'
#' Returns path to cell format definitions which is set in global options. To change this path see \link[xltabr]{set_cell_format_path}.
...
#'
#' Returns path to number format definitions which is set in global options. To change this path see \link[xltabr]{get_num_format_path}.
...
#'
#' Set the path to the style sheet to be used by package. To get this path see \link[xltabr]{get_style_path}.
...
set_style_path <- function(path = NULL){
  ...
  if(file.exists(path)) option(list(xltabr.style.path = path)) else stop("Speficied file path does not exist")
}

#' Set cell format path
#'
#' Set the path to the cell formats to be used by xltabr. To get this path see \link[xltabr]{get_cell_format_path}.
#' Default cell format used by package is [here](https://github.com/moj-analytical-services/xltabr/blob/dev/inst/extdata/style_to_excel_number_format.csv?raw=true).
...
set_cell_format_path <- function(path = NULL){
  ...
  if(file.exists(path)) option(list(xltabr.cell.format.path = path)) else stop("Speficied file path does not exist")
}

#' Set number format path
#'
#' Set the path to the number formats to be used by xltabr. To get this path see \link[xltabr]{get_num_format_path}.
...
set_num_format_path <- function(path = NULL){
  ...
  if(file.exists(path)) option(list(xltabr.number.format.path = path)) else stop("Speficied file path does not exist")
}
