...
Users of the xltabr package progressively build up an object called `tab`, which is just an R list.  For instance, we add titles to this object at `tab$titles`, and styles are stored in `tab$style_catalogue`.

A good place to start in `xltabr` is to generate a `tab` object and start exploring its contents, which you can do using the following code:
